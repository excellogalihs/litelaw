...
def search_duckduckgo(query):
    ...
    try:
        ...
        return "Error: The 'ddgs' library is required for live web searches. Please install it using: pip install ddgs"
...
_STOPWORDS = {
    ...
    "was", "were", "be", "been", "with", "at", "by", "it", "this", "that",
    "i", "you", "your", "my", "me", "do", "does", "did", "not", "no", "into",
    "as", "if", "so", "then", "than", "from", "when", "where", "how", "what",
}

def _tokenize(text):
    """Lowercase, punctuation-stripped word set, minus stopwords -- used for
    cheap keyword-overlap scoring (no embeddings needed on CPU-only hardware)."""
...
if __name__ == "__main__":
    ...
    # Load persistent memories + learned skills from store for CLI sessions
    _cli_memories = None
    _cli_skills = None
    ...
    if os.path.exists(_cli_store_path):
        try:
            with open(_cli_store_path, "r") as _f:
                ...
                _cli_skills = _store_data.get("skills")
        except Exception:
            pass
    
    session_messages = [{"role": "system", "content": get_system_prompt(_cli_memories, _cli_skills)}]
    ...
    while True:
        try:
            ...
            if user_input.lower() == 'clear':
                ...
                continue
            session_messages[0] = {"role": "system", "content": get_system_prompt(_cli_memories, _cli_skills, query=user_input)}
