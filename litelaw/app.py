...
function calcRenderPreview(){
  ...
  // Try to live-preview a result if the expression currently evaluates cleanly
  if(/[0-9]$/.test(calcExprStr)){
    ...
    } catch(e) { /* keep last valid value while mid-expression */ }
  }
}
...
async function fetchReminders(){
  ...
  } catch(e) { /* stay offline-friendly, keep last known reminders */ }
  renderCalendar();
  renderReminderPanel();
...
document.getElementById('addReminderBtn').addEventListener('click', async () => {
  ...
  if(res.ok){
    ...
    renderCalendar();
    renderReminderPanel();
    if(calSelectedDateKey === calDateKey(new Date())) calNotifiedKey = null; // re-notify since content changed
    checkTodayReminderNotification();
  }
});
...
// Surface today's reminders as an in-page banner and, where permitted, a native browser notification.
function checkTodayReminderNotification(){
  ...
  if(calNotifiedKey === todayKey) return; // already alerted for this date's current content
...
// Re-check reminders periodically so the banner/notification stays accurate across midnight rollovers.
