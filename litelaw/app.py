...
MULTI_AGENT_LOG_PREFIX = "MULTI_AGENT_LOG::"
...
def build_model_context(session_messages):
    """Strips logged multi-agent transcripts out of the message list before it's sent to
    the model as conversational context, since those are UI replay records, not real turns."""
    return [m for m in session_messages if not (m["role"] == "assistant" and m["content"].startswith(MULTI_AGENT_LOG_PREFIX))]
...
def web_run_agent(user_goal, session_messages, memories):
    ...
    for step in range(10):
        response = call_ollama(build_model_context(session_messages))
        if not response:
            steps.append({
                "type": "error",
...
def web_run_multi_agent(user_goal, session_messages, memories):
    """Runs the Planner -> Researcher/Coder/Executor -> Reviewer crew and logs the
    resulting step transcript into chat history so it can be replayed later."""
    steps = run_multi_agent(user_goal, memories)

    session_messages.append({"role": "user", "content": f"Task: {user_goal}"})
    session_messages.append({"role": "assistant", "content": MULTI_AGENT_LOG_PREFIX + json.dumps(steps)})
...
def get_chat_history():
    ...
    for idx, m in enumerate(raw_msgs):
        ...
        elif m["role"] == "assistant" and m["content"].startswith(MULTI_AGENT_LOG_PREFIX):
            try:
                logged_steps = json.loads(m["content"][len(MULTI_AGENT_LOG_PREFIX):])
                visible_steps.extend(logged_steps)
            except Exception:
                pass
...
def chat_multi():
    """Multi-agent variant: routes the goal through Planner -> Researcher/Coder/Executor -> Reviewer."""
    data = request.get_json(force=True) or {}
    chat_id = data.get("chat_id")
    message = (data.get("message") or "").strip()
    ...
    session_messages = store["chats"][chat_id]["messages"]
    steps = web_run_multi_agent(message, session_messages, store["memories"])
...
def convert_file():
    ...
    try:
        ...
        # DOCX -> PDF
            ...
            # Direct complex styling compilation natively in pure Python is limited without an OS layer backend.
            # We generate a structured PDF document mapping raw element paragraph contents as standard fallback engine.
            doc = Document(io.BytesIO(file_bytes))
            writer = PdfWriter()
            # Simple metadata mock generation to avoid system dependencies (LibreOffice/Unoconv)
            output_buffer.write(b"%PDF-1.4\n%Fallback implementation: Text elements harvested out into document frame.\n")
        ...
        # IMAGE transformations (PNG -> JPEG, JPEG -> PNG)
            ...
            # Normalize RGBA to RGB context for alpha channel constraints in JPEG targets
            ...
            out_mimetype = f"image/{target_format}"
...
.mode-toggle-btn{
  ...
  border:1px solid var(--border-soft); background:rgba(139,92,246,0.06); color:var(--text-dim);
  cursor:pointer; transition:all .15s ease; white-space:nowrap;
}
.mode-toggle-btn:hover{ border-color:var(--violet-1); color:var(--text-0); }
.mode-toggle-btn.active{
  background:linear-gradient(135deg, var(--violet-2), var(--magenta)); color:#fff;
  border-color:var(--violet-1); box-shadow:0 0 10px rgba(139,92,246,0.4);
}
.input-mode-row{ display:flex; justify-content:flex-end; max-width:800px; margin:0 auto; width:100%; padding:0 4px 8px; }
      ...
      <div class="stage-panel active" id="chatPanel">
        ...
        <div class="input-wrap">
          <div class="input-mode-row">
            <button class="mode-toggle-btn" id="multiAgentToggle" title="Toggle Multi-Agent Mode: Planner → Researcher/Coder/Executor → Reviewer">🤖 Multi-Agent Mode</button>
          </div>
...
function addStepNodeToStage(step) {
  ...
  if(step.type === 'user_msg') {
    ...
    if(badge) thoughtEl.appendChild(badge);
    thoughtEl.appendChild(document.createTextNode(step.text));
...
multiAgentToggle.addEventListener('click', () => {
  ...
  multiAgentToggle.textContent = multiAgentMode ? '🤖 Multi-Agent Mode: ON' : '🤖 Multi-Agent Mode';
});
...
async function dispatchMessage(){
  ...
  const thinkText = multiAgentMode ? '🤖 orchestrating agent crew (planner → researcher/coder/executor → reviewer)...' : '🤖 thinking...';
  thinkRow.innerHTML = `<div style="font-size:12px; color:var(--text-dim); padding:4px;">${thinkText}</div>`;
  chatContainer.appendChild(thinkRow);
  scrollToBottom(true);

  try {
    const endpoint = multiAgentMode ? '/api/chat/multi' : '/api/chat';
